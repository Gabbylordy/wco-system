...
interface SocietyThreat {
  ...
  recommendedActions: string[];
  status: 'active' | 'investigating' | 'held' | 'cleared';
  originalParcelData: any; // Store the original parcel data for details view
}
...
export function SocietyModule() {
  ...
  const [activeCategory, setActiveCategory] = useState<string | null>(null);
  const [threats, setThreats] = useState<SocietyThreat[]>([]);
  const [filteredThreats, setFilteredThreats] = useState<SocietyThreat[]>([]);
    ...
    const societyThreats: SocietyThreat[] = parcels
      .filter(parcel => {
        ...
        const hasDrugs = parcel.product_title?.toLowerCase().includes('pharma') || 
                         parcel.product_title?.toLowerCase().includes('drug') ||
                         parcel.product_title?.toLowerCase().includes('medicine') ||
                         parcel.product_title?.toLowerCase().includes('pill') ||
                         parcel.product_title?.toLowerCase().includes('tablet') ||
                         parcel.product_title?.toLowerCase().includes('capsule');
        const hasWildlife = parcel.product_title?.toLowerCase().includes('ivory') ||
                           parcel.product_title?.toLowerCase().includes('animal') ||
                           parcel.product_title?.toLowerCase().includes('endangered') ||
                           parcel.product_title?.toLowerCase().includes('skin') ||
                           parcel.product_title?.toLowerCase().includes('bone');
        const hasContraband = parcel.product_title?.toLowerCase().includes('counterfeit') ||
                             parcel.product_title?.toLowerCase().includes('fake') ||
                             parcel.description?.toLowerCase().includes('replica');
        
        return isHighRisk || hasWeapons || hasDrugs || hasWildlife || hasContraband || parcel.is_split_shipment;
      })
      .map((parcel, index) => {
        // Determine threat type
        let threatType: SocietyThreat['threatType'] = 'high-risk';
        if (parcel.risk_categories?.includes('WEAPONS')) {
          ...
          threatType = 'pharma';
                   ...
                   parcel.product_title?.toLowerCase().includes('bone')) {
          threatType = 'wildlife';
        } else if (parcel.product_title?.toLowerCase().includes('counterfeit') ||
                  parcel.product_title?.toLowerCase().includes('fake') ||
                  parcel.description?.toLowerCase().includes('replica')) {
          threatType = 'contraband';
        } else if (parcel.is_split_shipment) {
          threatType = 'split-shipment';
        }
        ...
        if (parcel.item_price_aed > 5000) indicators.push('High-value shipment');
        if (!parcel.importer_name || parcel.importer_name === 'Unknown') indicators.push('Unknown importer');
        if (parcel.hs_confidence_score < 0.7) indicators.push('Low HS code confidence');
        ...
        const riskScore = Math.min(95,
          ...
          (parcel.risk_categories?.length || 0) * 10 +
          (parcel.item_price_aed > 5000 ? 15 : 0) +
          (parcel.hs_confidence_score < 0.7 ? 10 : 0)
        );
        ...
        const documentStatus = {
          ...
          'Certificate of Origin': Math.random() > 0.7 ? 'verified' : 'missing',
          'Safety Data Sheet': threatType === 'pharma' ? 'missing' : 'pending',
          'Import License': threatType === 'weapons' ? 'missing' : 'pending',
          'CITES Permit': threatType === 'wildlife' ? 'missing' : 'not-applicable'
        };
        ...
        const recommendedActions = [
          'Physical inspection required',
          'Request additional documentation',
          threatType === 'pharma' ? 'Verify with health authorities' : 'Verify with source country authorities',
          threatType === 'weapons' ? 'Alert security agencies' : 'Sample testing for prohibited substances',
          'Cross-check with watchlist databases'
        ];

        return {
          id: `soc-${parcel.order_id || index + 1}`,
          orderId: parcel.order_id || `ORD-${index + 1}`,
          importerName: parcel.importer_name || 'Unknown Importer',
          productTitle: parcel.product_title || 'Unknown Product',
          description: parcel.description || '',
          ...
          recommendedActions,
          status: riskScore > 70 ? 'active' : riskScore > 50 ? 'investigating' : 'held',
          originalParcelData: parcel // Store original data for detailed view
        };
      });

    setThreats(societyThreats);
    setFilteredThreats(societyThreats);
    if (societyThreats.length > 0 && !selectedThreat) {
      setSelectedThreat(societyThreats[0].id);
    }
  }, [processedData]);
  ...
  useEffect(() => {
    ...
    // Filter by category
    if (activeCategory) {
      filtered = filtered.filter(t => t.threatType === activeCategory);
    }
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(t => 
        ...
        t.importerName.toLowerCase().includes(query) ||
        t.productTitle.toLowerCase().includes(query) ||
        t.hsCode.includes(query) ||
        t.countryOrigin.toLowerCase().includes(query)
      );
    }
    
    setFilteredThreats(filtered);
    setCurrentPage(1); // Reset to first page when filters change
  }, [threats, activeCategory, searchQuery]);
  ...
  const threatCategories = [
    ...
    { 
      id: 'wildlife', 
      label: 'Wildlife/Endangered', 
      icon: AlertTriangle, 
      count: threats.filter(t => t.threatType === 'wildlife').length 
    },
    { 
      id: 'contraband', 
      label: 'Contraband Goods', 
      icon: Package, 
      count: threats.filter(t => t.threatType === 'contraband').length 
    },
    { 
      id: 'high-risk', 
      label: 'High-Risk Goods', 
  ...
  const handleThreatAction = (action: 'hold' | 'alert' | 'sample' | 'approve' | 'reject' | 'escalate') => {
    if (!selected) return;

    let message = '';
    switch (action) {
      case 'hold':
        message = `Hold issued for shipment ${selected.orderId}`;
        toast.error("Hold issued", {
          description: message,
        });
        break;
      case 'alert':
        message = `Partner agencies alerted about ${selected.orderId}`;
        toast.warning("Agencies alerted", {
          description: message,
        });
        break;
      case 'sample':
        message = `Lab sample requested for ${selected.productTitle}`;
        toast.info("Sample requested", {
          description: message,
        });
        break;
      case 'approve':
        message = `Parcel ${selected.orderId} cleared for ${selected.threatType}`;
        toast.success("Approved!", { description: message });
        break;
      case 'reject':
        message = `Parcel ${selected.orderId} rejected - Requires further inspection`;
        toast.error("Rejected!", { description: message });
        break;
      case 'escalate':
        message = `Parcel ${selected.orderId} escalated to supervisor`;
        toast.warning("Escalated!", { description: message });
        break;
    }
  };

  const handleViewDetails = (threat: SocietyThreat) => {
    setSelectedThreat(threat.id);
    setViewMode('view');
    setIsDialogOpen(true);
  };

  const handleReviewThreat = (threat: SocietyThreat) => {
    setSelectedThreat(threat.id);
    setViewMode('review');
    setIsDialogOpen(true);
  };
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `society_threats_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    toast.success("Export Complete", {
      description: `Exported ${threats.length} threats to CSV`,
    });
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-6">
          ...
          <StatCard
            title="Held Shipments"
            value={stats.heldShipments.toLocaleString()}
            subtitle="Pending verification"
            icon={Package}
            variant="warning"
          />
          <StatCard
            title="High Value Threats"
            value={stats.highValueThreats.toLocaleString()}
            subtitle="Value > AED 5,000"
            icon={Shield}
            variant="danger"
          />
          <StatCard
            title="Interdictions (MTD)"
            value={stats.interdictions.toLocaleString()}
            subtitle="Successfully stopped"
            icon={Shield}
            variant="success"
          />
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input
                placeholder="Search threats by order ID, importer, product, or HS code..."
                className="pl-10"
                value={searchQuery}
            ...
            <div className="lg:col-span-1 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">
                  {filteredThreats.length.toLocaleString()} threats
                </span>
              </div>
              ...
              <div className="space-y-3 max-h-[calc(100vh-480px)] overflow-y-auto pr-2">
                {paginatedThreats.map((threat) => (
                  <div
                    key={threat.id}
                    className="glass-card rounded-lg p-4 cursor-pointer transition-all border border-border/50 hover:border-red-500/50"
                  >
                    <div className="flex items-start gap-3">
                      <RiskScoreRing score={threat.riskScore} size="sm" />
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          <RiskBadge severity="critical" pulse>
                            {threat.threatType === 'weapons' ? 'Weapons' : 
                             threat.threatType === 'pharma' ? 'Pharma' : 
                             threat.threatType === 'wildlife' ? 'Wildlife' :
                             threat.threatType === 'contraband' ? 'Contraband' :
                             threat.threatType === 'split-shipment' ? 'Split' : 'High-Risk'}
                          </RiskBadge>
                        </div>
                        <p className="text-xs text-muted-foreground truncate">
              ...
              {filteredThreats.length > itemsPerPage && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t">
                  <div className="text-sm text-muted-foreground">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredThreats.length)} of {filteredThreats.length.toLocaleString()} threats
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                ...
                <div className="glass-card rounded-xl p-6 border border-border/50">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <RiskBadge severity="critical" pulse>
                          {selected.threatType === 'weapons' ? 'Weapons Threat' : 
                           selected.threatType === 'pharma' ? 'Pharma Threat' : 
                           selected.threatType === 'wildlife' ? 'Wildlife Threat' :
                           selected.threatType === 'contraband' ? 'Contraband Threat' :
                           selected.threatType === 'split-shipment' ? 'Split Shipment' : 'High-Risk Goods'}
                        </RiskBadge>
                      </div>
                      <p className="text-sm text-muted-foreground">{selected.productTitle}</p>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <p className="text-xs text-muted-foreground">HS Code</p>
                        <p className="font-medium font-mono">{selected.hsCode}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Value</p>
                        <p className="font-medium text-red-600">AED {selected.valueAED.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                      </div>
                      <div>
                        <p className="text-xs text-muted-foreground">Origin</p>
                        <p className="font-medium">{selected.countryOrigin}</p>
                      ...
                      <div className="col-span-3">
                        ...
                        <p className="text-sm font-medium">{selected.importerName}</p>
                      </div>
                      <div className="col-span-3">
                        <p className="text-xs text-muted-foreground">Description</p>
                        <p className="text-sm">{selected.description || 'No description available'}</p>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <Button 
                      ...
                      onClick={() => handleThreatAction('hold')}
                    >
                      Issue Hold
                    </Button>
                    <Button 
                      variant="outline"
                      onClick={() => handleThreatAction('alert')}
                    >
                      Alert Partner Agencies
                    </Button>
                    <Button 
                      variant="outline"
                      onClick={() => handleThreatAction('sample')}
                    >
                      Request Lab Sample
                    </Button>
                    <Button
      ...
      {selected && (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {viewMode === 'review' ? 'Threat Review' : 'Threat Details'}
              </DialogTitle>
              <DialogDescription>
                {viewMode === 'review' 
                  ? `Complete analysis of threat ${selected.orderId} - Take action below`
                  : `Complete information for threat ${selected.orderId}`
                }
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-2">
                <Label className="font-semibold">Threat Type</Label>
                <div className="text-sm">
                  <RiskBadge severity="critical">
                    {selected.threatType.replace('-', ' ').toUpperCase()}
                  </RiskBadge>
                </div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm font-medium">AED {selected.valueAED.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label className="font-semibold">Country Origin</Label>
                <div className="text-sm">{selected.countryOrigin}</div>
              </div>
              <div className="space-y-2 col-span-2">
                <Label className="font-semibold">Description</Label>
                <div className="text-sm text-muted-foreground">{selected.description || 'No description available'}</div>
              </div>
              
              {/* Document Status */}
              <div className="space-y-2 col-span-2">
                <Label className="font-semibold">Document Verification</Label>
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(selected.documentStatus).map(([doc, status]) => {
                    let bgColor = '';
                    let textColor = '';
                ...
                <div className="space-y-2 col-span-2">
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{selected.originalParcelData.is_split_shipment ? 'Yes' : 'No'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Duty Applicable:</span>
                      <span>{selected.originalParcelData.duty_applicable ? 'Yes' : 'No'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Duty Payable:</span>
                      <span>AED {selected.originalParcelData.duty_payable_aed?.toFixed(2) || '0.00'}</span>
                    </div>
                    <div className="flex justify-between">
            ...
            {viewMode === 'review' ? (
              <DialogFooter className="flex gap-2 pt-4 border-t">
                <Button
                  variant="destructive"
                  onClick={() => handleThreatAction('reject')}
                  className="gap-2"
                >
                ...
                <Button
                  variant="outline"
                  onClick={() => handleThreatAction('escalate')}
                ...
                <Button
                  variant="default"
                  onClick={() => handleThreatAction('approve')}
